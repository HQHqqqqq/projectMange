<template>
  <div class="list-container">
    <header class="list-header">
      <div class="list-header-myreview">
        <el-row :gutter="20">
          <el-col :md="24" :lg="24">
            <el-row class="btn-item">
              <el-button type="primary" :disabled="isEditor" @click="reviewEditor" size="mini">编辑</el-button>
              <el-button type="primary" :disabled="isRecall" @click="reviewRecall" size="mini">撤回</el-button>
    ...
    <!---待我审核的项目弹框-->
    <el-dialog title="登记审批" :visible.sync="examineDialog" class="dialog">
      <el-table :data="tableData1"
        ...
        <el-table-column prop="name" label="审核内容" width="180">
          <template slot-scope="scope">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="confirmPass('2')" size="small" disabled>驳 回</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      //撤销状态，如果是撤销状态跳转到合同页面提交按钮不显示 撤销是1，其他是2
      isRevoke: null
    };
  },
  filters: {
  ...
  methods: {
    ...
    getProjectTable(val) {
      ...
      } else if (this.cacheParams.code != "myWait") {
        params = params;
      }
      //获取项目列表信息
    ...
    handleSelectionChange(val) {
      ...
      localStorage.setItem("businessProjectId", this.multipleSelection.id);
    },
    ...
    reviewEditor() {
      ...
      if (this.multipleSelection.auditStatus == "撤销") {
        sessionStorage.setItem("isRevoke", 1);
      } else {
    ...
    getProjectMsyById() {
      ...
      if (jumpType == 1) {
        //isRevoke 是否撤销状态，如果是撤销状态跳转到商机合同或者子项目的时候提交按钮隐藏
        this.$router.push({
