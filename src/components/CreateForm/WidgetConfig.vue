<template>
  <div v-if="show">
    <el-form label-position="top">
      <el-form-item label="Table Header" v-if="data.type =='table'">
        <el-row>
          <el-col :span="24">
            <div class="grid-content bg-purple-dark">{{data.options.defaultValue}}</div>
          </el-col>
        </el-row>
        <el-form-item label="添加行" v-if="data.type!='grid'">
      ...
      <el-form-item label="url" v-if="data.type!='grid'">
        <el-input v-model="data.url"></el-input>
      </el-form-item>
      <el-form-item
        label="占位内容"
        v-if="Object.keys(data.options).indexOf('placeholder')>=0 && (data.type!='time' || data.type!='date') && data.type != 'row'"
      >
        <el-input v-model="data.options.placeholder"></el-input>
      </el-form-item>
      <el-form-item label="占位内容" v-if="data.type == 'row'">
        <el-input v-model="data.options.placeholder"></el-input>
      </el-form-item>

      <el-form-item v-if="Object.keys(data.options).indexOf('size')>=0" label="大小">
        宽度：
        <el-input v-model.number="data.options.size.width" style="width: 90px;" type="number" />高度：
        <el-input v-model.number="data.options.size.height" style="width: 90px;" type="number" />
      </el-form-item>

      <el-form-item
        v-if="Object.keys(data.options).indexOf('placeholder')>=0 && (!['time','date'].includes(data.type))"
        label="占位内容"
      >
          ...
          <template v-if="data.type=='cascader' ">
            ...
            >
              <span slot-scope="{ node, data }" class="custom-tree-node">
                <span>
                  <el-input v-model="data.label" style="width:50px;" size="mini" />
                  <el-input v-model="data.value" style="width:50px;" size="mini" />
                </span>
                <span>
                  <i class="drag-item" style="font-size: 16px;margin: 0 5px;cursor: move;">
                    <icon name="bars" />
      ...
      <el-form-item
        v-if="Object.keys(data.options).indexOf('defaultValue')>=0 && (data.type == 'textarea' || data.type == 'textarea2' || data.type == 'input' || data.type == 'input2' || data.type=='rate' || data.type=='color' || data.type=='switch')"
        label="默认值"
      >
        <el-input
          v-if="data.type=='textarea'|| data.type=='textarea2'"
          :rows="5"
          v-model="data.options.defaultValue"
          type="textarea"
        />
        <el-input v-if="data.type=='input'||data.type=='input2'" v-model="data.options.defaultValue" />
        <el-rate
          v-if="data.type == 'rate'"
      ...
      <template v-if="data.type=='upload'">
        ...
        <el-form-item :required="true" label="action上传的地址">
...
export default {
  ...
  watch: {
    ...
    // cyh修改是否可监听属性
    "data.options.listened": function(val) {
      debugger;
      if (val) {
        this.data.options.listened = true;
      } else {
        this.data.options.listened = false;
      }
    }
  },
  methods: {
    ...
    //cyh 添加监听input
    handleListenOptionRemove(index) {
      debugger;
      this.data.options.listen.splice(index, 1);
    },
    handleAddListen() {
      this.data.options.listen.push({
        value: ""
      });
    },
    handleAddOption() {
      if (this.data.options.showLabel) {
        ...
        this.data.options.options.push({ label: "new_", value: key });
      } else {
        this.data.options.options.push({
          value: "新选项"
        });
    ...
    append(data) {
      ...
      const key =
        Date.parse(new Date()) + "_" + Math.ceil(Math.random() * 99999);
      const newChild = { label: "new_", value: key };
      if (!data.children) {
    ...
    remove(node, data) {
      ...
      // this.$refs.tree.updateKeyChildren(keys, parent.data)
    }
  }
};
</script>
