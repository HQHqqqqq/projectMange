<template>
  <div class="mounth-output">
    <el-row :gutter="10" class="search" type="flex" justify="space-between">
      <el-col :xs="8" :sm="6" :md="8" :lg="7" :xl="7">
        <div class="wrap">
          ...
          <el-select v-model="firstSelectvalue" size="mini" placeholder="请选择" @change="changeSearch"
            class="search-select">
    ...
    <el-table :data="tableData" :height="asideHeight" border :header-cell-style="{
        fontSize: '14px',
        color: '#303133',
        height: '51px',
        padding: 0,
      ...
      <el-table-column prop="projectNumber" label="项目编号" align="center" :width="180"></el-table-column>
      ...
      <el-table-column prop="implementationDepartmentC" label="实施C级单位" align="center" :width="130">
        <template slot-scope="scope">
          <span>{{ scope.row.implementationDepartmentC|formatIsNull }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="residualValue" align="center" label="剩余产值">
        <template slot-scope="scope">
          <span>{{ scope.row.residualValue|formatIsNull }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="contractNumbers" align="center" label="合同名称" :width="160">
          <template slot-scope="scope">
          <span>{{ scope.row.contractNumbers|formatIsNull }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="saleDepartmentB" align="center" label="销售B级单位" :width="130">
        <template slot-scope="scope">
          <span>{{ scope.row.saleDepartmentB|formatIsNull }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="saleDepartmentC" align="center" label="销售C级单位" :width="130">
        <template slot-scope="scope">
          <span>{{ scope.row.saleDepartmentC|formatIsNull }}</span>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    refresh() {
      ...
      this.getAssociatedOutputList(params);
    },
    ...
    getAssociatedOutputList(val) {
      //获取项目列表信息
      getAssociatedOutput(
        `/sgpm/rest/api/server/project/monthly/output/associated`,
        val
      )
        .then(res => {
          if (res.status == 200) {
            ...
            result.forEach((ele, index) => {
              if (ele.contractNumbers&&ele.contractNumbers.indexOf(";") > -1) {
                ele.contractNumbers = ele.contractNumbers.split(";");
              } else if (
                ele.contractNumbers&&ele.contractNumbers.indexOf(";") == -1 &&
                ele.contractNumbers != ""
              ) {
                var ary = [];
    ...
    search() {
      ...
      this.getAssociatedOutputList(param);
    },
